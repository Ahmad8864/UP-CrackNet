...
class Generator(torch.nn.Module):
    def __init__(self, input_dim, num_filter, output_dim):
        ...
        self.conv6 = ConvBlock(num_filter * 8, num_filter * 8)  # out = (bn, 512, 4, 4)
        self.conv7 = ConvBlock(num_filter * 8, num_filter * 8)  # out = (bn, 512, 2, 2)
        self.conv8 = ConvBlock(num_filter * 8, num_filter * 8, batch_norm=False)    # out = (bn, 512, 1, 1)
        # Decoder
        self.deconv1 = DeconvBlock(num_filter * 8, num_filter * 8, dropout=True)    # out = (bn, 512, 2, 2)
        self.deconv2 = DeconvBlock(num_filter * 8 * 2, num_filter * 8, dropout=True)    # out = (bn, 512, 4, 4)
        self.deconv3 = DeconvBlock(num_filter * 8 * 2, num_filter * 8, dropout=True)    # out = (bn, 512, 8, 8)
        self.deconv4 = DeconvBlock(num_filter * 8 * 2, num_filter * 8)  # out = (bn, 512, 16, 16)
...
class Generator128(torch.nn.Module):
    def __init__(self, input_dim, num_filter, output_dim):
        ...
        # Encoder
        # input image size = 128, guess by nachuan
        # out = (bn, filters, H, W)
        self.conv1 = ConvBlock(input_dim, num_filter, activation=False, batch_norm=False)   # (bn, 64, 64, 64)
        self.conv2 = ConvBlock(num_filter, num_filter * 2)  # (bn, 128, 32, 32)
        self.conv3 = ConvBlock(num_filter * 2, num_filter * 4)  # (bn, 256, 16, 16)
        self.conv4 = ConvBlock(num_filter * 4, num_filter * 8)  # (bn, 512, 8, 8)
        self.conv5 = ConvBlock(num_filter * 8, num_filter * 8)  # (bn, 512, 4, 4)
        self.conv6 = ConvBlock(num_filter * 8, num_filter * 8)  # (bn, 512, 2, 2)
        self.conv7 = ConvBlock(num_filter * 8, num_filter * 8, batch_norm=False)    # (bn, 512, 1, 1)
        # Decoder
        self.deconv1 = DeconvBlock(num_filter * 8, num_filter * 8, dropout=True)    # (bn, 512, 2, 2)
        # U-Net Architecture
        self.deconv2 = DeconvBlock(num_filter * 8 * 2, num_filter * 8, dropout=True)    # (bn, 512, 4, 4)
        self.deconv3 = DeconvBlock(num_filter * 8 * 2, num_filter * 8, dropout=True)    # (bn, 512, 8, 8)
        self.deconv4 = DeconvBlock(num_filter * 8 * 2, num_filter * 4)  # (bn, 256, 16, 16)
        self.deconv5 = DeconvBlock(num_filter * 4 * 2, num_filter * 2)  # (bn, 128, 32, 32)
        self.deconv6 = DeconvBlock(num_filter * 2 * 2, num_filter)  # (bn, 64, 64, 64)
        self.deconv7 = DeconvBlock(num_filter * 2, output_dim, batch_norm=False)    # (bn, 3, 128, 128)
